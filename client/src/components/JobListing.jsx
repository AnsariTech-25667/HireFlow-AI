...
const JobListing = () => {
  ...
  const activeFiltersCount =
    ...
    (searchFilter.title ? 1 : 0) +
    (searchFilter.location ? 1 : 0);
        ...
        <AnimatePresence>
          {displayedJobs.map((job, index) => (
            <motion.div
              key={`${job._id}-${index}`}
              ref={
                index === displayedJobs.length - 1 ? lastJobElementRef : null
              }
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -20 }}
              transition={{
                duration: 0.3,
                delay: (index % JOBS_PER_PAGE) * 0.05,
              }}
      ...
      {displayedJobs.length === 0 && !isLoading && (
        ...
        >
          <div className="glass rounded-2xl p-12 max-w-md mx-auto">
            ...
            <button onClick={clearAllFilters} className="btn-primary px-6 py-2">
              Clear all filters
            </button>
          </div>
        </motion.div>
