...
const AchievementSystem = () => {
      ...
      if (statValue >= template.requirement.value) {
        ...
        setTimeout(() => {
          ...
          setAchievements(prev => 
            prev.map(a => a.id === newAchievement.id ? { ...a, isNew: false } : a)
          )
        }, 5000)
      }
    })
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (Math.random() > 0.7) { // 30% chance to update stats
        setUserStats(prev => ({
          ...prev,
          applicationsSubmitted: prev.applicationsSubmitted + (Math.random() > 0.8 ? 1 : 0),
          profileCompletion: Math.min(100, prev.profileCompletion + (Math.random() > 0.9 ? 5 : 0)),
          interviewsScheduled: prev.interviewsScheduled + (Math.random() > 0.95 ? 1 : 0),
          skillsAssessed: prev.skillsAssessed + (Math.random() > 0.85 ? 1 : 0),
          connectionsAdded: prev.connectionsAdded + (Math.random() > 0.8 ? 1 : 0),
            ...
            <div className="glass rounded-2xl p-6 border border-white/20 shadow-2xl backdrop-blur-xl max-w-sm">
              <div className="text-center">
                <motion.div
                  ...
                  className="mb-4"
                >
                  <div className={`w-16 h-16 mx-auto rounded-full bg-gradient-to-r ${showNotification.color} flex items-center justify-center`}>
          ...
          <div className="space-y-2">
            {achievements.slice(-3).reverse().map((achievement) => {
              ...
              return (
                ...
                >
                  <div className={`w-6 h-6 rounded-lg bg-gradient-to-r ${achievement.color} flex items-center justify-center flex-shrink-0`}>
